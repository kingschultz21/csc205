...
# Commentary:
#   
#   CSC 205 Assignment 4
#	Submitted by: Connor Schultz (V00872923)
#   Date of Submission: Oct. 30th, 2018
#
#
...
# Dummy variable to prevent crashing
TODO = None
...
class block(object):
    ...
    def modify_velocity(self, dv):
        ...
        The function should modify velocity according to dv, and check if it is
        within the allowed velocity limit, and clip to maximum velocity if
        necessary. 
        ...
        if v1 > MAX_V:
            ...
            self.velocity[1] = MAX_V
        elif v1 < MAX_V * -1:
            self.velocity[0] = MAX_V * -1
        elif v2 < MAX_V * -1:
            self.velocity[1] = MAX_V * -1
        else:
    ...
    def update(self):
        ...
        # Update position
        self.center = self.center+ (self.velocity / FPS)
        # Update velocity
    ...
    def draw(self, canvas):
        ...
        if self.obj_type == "block":
            ...
            # where the ball should have AABB equal to the "block" case.
            	...
            	#Note to marker:
          			#Drawing the circle is handled in the main method.
          			#I could not get it to work using this format
...
def solve_direction(pair, obj_list):
    ...
    mask: numpy array of size 2
        ...
        should be updated. This was briefly explained in Lecture 17.

    """
    print(pair)
    
    if pair[0] == 0 and pair[1] == 4:
    	mask = np.array([1.0, 0.0])
    elif pair[0] == 1 and pair[1] == 4:
    	mask = np.array([1.0, 0.0])
    elif pair[0] == 2 and pair[1] == 4:
    	mask = np.array([0.0, 1.0])
    elif pair[0] == 3 and pair[1] == 4:
    	mask = np.array([0.0, 1.0])
    else:
...
def init():
    ...
    obj_list += [
        block(
            ...
            obj_type="block"),
        block(
            center=(CANVAS_SIZE+10,CANVAS_SIZE//2),
            width=10,
            height=CANVAS_SIZE,
            color=(0,0,255),
            mass=-1,
        ...
        block(
            ...
            obj_type="block"),
        block(
            center=(CANVAS_SIZE//2,CANVAS_SIZE+10),
            width=CANVAS_SIZE,
            height=15,
            color=(0,0,255),
            mass=-1,
    ...
    #Note to Marker:
    #I modify the velocitys to make the throws feel stronger
    x_bias = 3
    y_bias = 6
    velocity = ((CUR_X-INIT_X)*x_bias,(CUR_Y-INIT_Y)*y_bias)
    ...
    #Add Corner Blocks to try and Fix corner issues
    #Not the prettiest solution but it seems to fix most of the issues
    ...
    obj_list += [
        ...
        block(
            ...
            obj_type="block"),
        block(
            center=(CANVAS_SIZE,CANVAS_SIZE),
            width=12,
            height=12,
            color=(255,255,255),
            mass=-1,
...
def main():
    ...
    while True:
        ...
        for obj in obj_list:
        	if obj == obj_list[4]:
                #Draw the Ball and do not draw the AABB
        		cv2.circle(canvas,(int(obj.center[0]),int(obj.center[1])),BALL_SIZE//2,(255,0,0),-1,cv2.LINE_AA)
        		break
        	obj.draw(canvas)
        ...
        if MOUSE_DOWN:
        	...
        	if pt_count == 0:
        		...
        		pt_count = 1
        	cv2.arrowedLine(canvas,PT1,(int(CUR_X),int(CUR_Y)),(0,0,255),5,cv2.LINE_AA)
        else:
        	pt_count = 0
        if mouse_down:
        	mouse_down = False
